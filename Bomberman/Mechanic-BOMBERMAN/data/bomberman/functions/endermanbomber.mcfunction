...
tag @s add endermanbomber
weather clear 1d
gamemode adventure @s
execute at @s run playsound item.trident.return master @s ~ ~ ~ 10 1.5
effect give @s minecraft:resistance 2 50 true
item replace entity @s hotbar.4 with smoker{display:{Name:'[{"text":"Enderman Bomb","italic":false,"color":"dark_purple","bold":true}]'},CanPlaceOn:[pink_terracotta]} 1
